...
function TransactionFirst({ user }: { user: any }) {
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetUserBankDetails(user._id as string);
  ...
  if (bankDetails?.data.length === 0 && !isLoading && !error)
    return (
      <div
        className=" cursor-pointer text-white border-2 border-green-600 p-4  rounded-[4px] m-2 font-semibold text-[20px] "
        onClick={() => {
  ...
  if (bankDetails?.data.length > 0 && !isLoading)
    return (
